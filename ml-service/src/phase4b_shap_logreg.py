"""
Phase 4b - Analyse SHAP sur LogisticRegression (candidat alternatif au
champion NB de la Phase 3), pour comparer la coherence metier des deux
interpretations avant la decision finale de la Phase 5.

LogisticRegression est un modele lineaire : shap.LinearExplainer donne des
valeurs SHAP EXACTES (pas d'approximation Kernel/Monte-Carlo), donc on peut
utiliser un echantillon plus large et un calcul quasi instantane.
"""
import numpy as np
import joblib
import shap
import matplotlib

matplotlib.use("Agg")
import matplotlib.pyplot as plt
from pathlib import Path
import scipy.sparse as sp

ROOT = Path(__file__).resolve().parents[1]
DATA_DIR = ROOT / "data"
MODELS_DIR = ROOT / "models"
REPORTS_DIR = ROOT / "reports"
REPORTS_DIR.mkdir(exist_ok=True)

MODEL_NAME = "LogisticRegression"

model = joblib.load(MODELS_DIR / f"model_{MODEL_NAME}.joblib")
tfidf = joblib.load(MODELS_DIR / "tfidf_vectorizer.joblib")
university_encoder = joblib.load(MODELS_DIR / "university_encoder.joblib")
category_encoder = joblib.load(MODELS_DIR / "category_encoder.joblib")
label_encoder = joblib.load(MODELS_DIR / "label_encoder.joblib")
class_names = list(label_encoder.classes_)

feature_names = (
    list(tfidf.get_feature_names_out())
    + [f"univ={c}" for c in university_encoder.categories_[0]]
    + [f"cat={c}" for c in category_encoder.categories_[0]]
)

X_train = sp.load_npz(DATA_DIR / "X_train.npz")
X_test = sp.load_npz(DATA_DIR / "X_test.npz")
y_test = np.load(DATA_DIR / "y_test.npy")

rng = np.random.RandomState(42)
BACKGROUND_SIZE = 100
EXPLAIN_SIZE = 200

bg_idx = rng.choice(X_train.shape[0], size=BACKGROUND_SIZE, replace=False)
background = X_train[bg_idx].toarray()

explain_idx = rng.choice(X_test.shape[0], size=EXPLAIN_SIZE, replace=False)
X_explain = X_test[explain_idx].toarray()
y_explain = y_test[explain_idx]

print("Calcul des valeurs SHAP exactes (LinearExplainer)...")
explainer = shap.LinearExplainer(model, background)
shap_values = explainer.shap_values(X_explain)
shap_values = np.array(shap_values)
print(f"shap_values raw shape: {shap_values.shape}")
if shap_values.shape[0] == len(class_names):
    shap_values = np.transpose(shap_values, (1, 2, 0))
print(f"shap_values shape: {shap_values.shape}")

np.save(DATA_DIR / "shap_values_logreg.npy", shap_values)

for c_idx, c_name in enumerate(class_names):
    plt.figure(figsize=(8, 6))
    shap.summary_plot(
        shap_values[:, :, c_idx],
        X_explain,
        feature_names=feature_names,
        show=False,
        max_display=20,
    )
    plt.title(f"SHAP summary (LogReg) - classe {c_name}")
    plt.tight_layout()
    out = REPORTS_DIR / f"shap_logreg_summary_{c_name}.png"
    plt.savefig(out, dpi=150)
    plt.close()
    print(f"Summary plot sauvegarde -> {out}")

expected_value = explainer.expected_value
if not hasattr(expected_value, "__len__"):
    expected_value = [expected_value]

for pos in [0, 1, 2]:
    pred_class_idx = model.predict(X_explain[pos : pos + 1])[0]
    true_class = y_explain[pos]
    expl = shap.Explanation(
        values=shap_values[pos, :, pred_class_idx],
        base_values=expected_value[pred_class_idx],
        data=X_explain[pos],
        feature_names=feature_names,
    )
    plt.figure(figsize=(9, 6))
    shap.plots.waterfall(expl, max_display=15, show=False)
    plt.title(
        f"Waterfall LogReg #{pos} - predit={class_names[pred_class_idx]} "
        f"(vrai={class_names[true_class]})"
    )
    plt.tight_layout()
    out = REPORTS_DIR / f"shap_logreg_waterfall_{pos}.png"
    plt.savefig(out, dpi=150)
    plt.close()
    print(f"Waterfall plot sauvegarde -> {out}")

print("\n" + "=" * 70)
print("SYNTHESE (LogisticRegression) - top features par classe (moy |SHAP|)")
print("=" * 70)
for c_idx, c_name in enumerate(class_names):
    mean_abs = np.abs(shap_values[:, :, c_idx]).mean(axis=0)
    top_idx = np.argsort(-mean_abs)[:15]
    print(f"\n--- {c_name} ---")
    for i in top_idx:
        print(f"  {feature_names[i]:35s} {mean_abs[i]:.5f}")

# proportion de l'importance totale portee par le texte (TF-IDF) vs metadata
n_tfidf = len(tfidf.get_feature_names_out())
for c_idx, c_name in enumerate(class_names):
    mean_abs = np.abs(shap_values[:, :, c_idx]).mean(axis=0)
    text_share = mean_abs[:n_tfidf].sum() / mean_abs.sum()
    print(f"Part de l'importance totale portee par le texte (TF-IDF) - {c_name}: {text_share:.1%}")
